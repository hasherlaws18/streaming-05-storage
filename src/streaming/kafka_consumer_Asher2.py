...
Terminal command to run this file from the root project folder:
    ...
    uv run python -m streaming.kafka_consumer_Asher2
...
from streaming.data_validation.data_contract_Asher import (
    CONSUMED_FIELDNAMES,
...
OUTPUT_CSV: Final[Path] = OUTPUT_DIR / "consumed_sales-Asher2.csv"
OUTPUT_DB: Final[Path] = OUTPUT_DIR / "sales-Asher2.duckdb"
...
def process_message(
    ...
    *,
    ...
    if stats.count > 0 and enriched["total"] < stats.minimum:
        enriched["new_lowest_sale"] = "Yes"
        LOG.info(
            f"NEW LOWEST SALE! ${enriched['total']:.2f} order={enriched['order_id']}"
        )
    else:
        enriched["new_lowest_sale"] = "No"
